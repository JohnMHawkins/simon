...
def LOG(msg):
    print(msg)
    print("******************************************")
    print("Weights: " + str(signals))
    print("Score: " + finalsscore)
    print("******************************************")
...
# spots (11 chan) at a036, a047 
DMX_SPOTS = [36, 47]
dmx_spots_curpos = [0,0]
dmx_spots_curdir = [0,0]
dmx_spots_lastupdate = [0.0, 0.0]
...
def DMXMoveSpots():
    ...
    try:
        ...
        LOG("Exception in dmx DMXMovespots")


def _dmxSpotsOn(color):
    if dmx == None:
        return

    try:
        for spot in DMX_SPOTS:
            if color == SIMON_RED:
                dmx.set_channel(spot + 7, 255 )
                dmx.set_channel(spot + 8, 0 )
                dmx.set_channel(spot + 9, 0 )
                dmx.set_channel(spot + 10, 0 )
            elif color == SIMON_GREEN:
                dmx.set_channel(spot + 7, 0 )
                dmx.set_channel(spot + 8, 255 )
                dmx.set_channel(spot + 9, 0 )
                dmx.set_channel(spot + 10, 0 )
            elif color == SIMON_BLUE:
                dmx.set_channel(spot + 7, 0 )
                dmx.set_channel(spot + 8, 0 )
                dmx.set_channel(spot + 9, 255 )
                dmx.set_channel(spot + 10, 0 )
            elif color == SIMON_YELLOW:
                dmx.set_channel(spot + 7, 255 )
                dmx.set_channel(spot + 8, 128 )
                dmx.set_channel(spot + 9, 0 )
                dmx.set_channel(spot + 10, 0 )

        # submit is handled by the caller
    except:
        LOG("Exception in dmx _dmxSpotsOn")


def _dmxSpotsOff(color):
    if dmx == None:
        return
    ...
    try:
        ...
        LOG("Exception in dmx _dmxSpotsOff")
...
def evaluateChoice(ts):
    ...
    if bOk:
        ...
        if curStep < len(curSequence):
            ...
            nextState = STATE_COMPUTER
            delay = 2.5
            finalsscore = "current score = " + str(len(curSequence))
        ...
        LOG("------------")
        finalsscore = "final Score = " + str(len(curSequence))
