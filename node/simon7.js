...
function sendDMX(buttonId, cmd, data) {
    ...
    if ( buttonId < dmxControllers.length ) {
       ...
       try {
         ...
         // just continue
         LOG(LOG_ERROR, "failed to send http message to dmx");
       }
...
function sendCommand(buttonId, cmd, data) {
  ...
  switch ( buttonId) {
    ...
    case SIMON_YELLOW:
      ...
      break;
    default:
        LOG(LOG_DEBUG, "unknown buttonId " + buttonId);
  }
...
function receiveSerialData(comName, cmd) {
  ...
  if ( cmdparts[0].indexOf("C") > -1 ) {
    ardId = SIMON_CENTER;
  } else if ( cmdparts[0].indexOf("R") > -1 ) {
    ardId = SIMON_RED;
  } else if ( cmdparts[0].indexOf("G") > -1 ) {
    ardId = SIMON_GREEN;
  } else if ( cmdparts[0].indexOf("B") > -1 ) {
    ardId = SIMON_BLUE;
  } else if ( cmdparts[0].indexOf("Y") > -1)  {
    ardId = SIMON_YELLOW;
  }

  if ( cmdparts.length > 1 && ardId > 0) {
    if (cmdparts[1].indexOf("W") > -1) {
      if (cmdparts.length > 2) {
        weight = parseInt(cmdparts[2])
        LOG(LOG_DEBUG, "...get weight " + weight.toString() + " for " + ardId.toString());
...
function startPlayersTimer() {
  ...
  setTimeout(function(){
    ...
    readAllButtons(false);
    // uncomment to run self-test sequence
    if ( autotest ) {
      testFakeButtonPress(playerSequence.length < testlength);
    }
...
function makePlayersChoice() {
    LOG(LOG_DEBUG, "makeplayerschoice");
...
function loop () {
  ...
  switch( gameState) {
    ...
    case GS_ATTRACT:
      LOG(LOG_DEBUG, "GS_ATTRACT waiting for red button");
